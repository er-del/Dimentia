...
class DimendiaPipeline:
    ...
    def convert(
        ...
        *,
        ...
        log.info(
            "input %sx%s @ %.2ffps (%s frames) mode=%s render=%s",
            out_w,
            out_h,
            meta.fps,
            meta.n_frames,
        ...
        tmp_video.close()
        writer = VideoWriter(tmp_video.name, meta.fps)
        depth_writer: VideoWriter | None = None
        if depth_preview_path is not None:
            depth_writer = VideoWriter(depth_preview_path, meta.fps)
        ...
        try:
            for i, frame in enumerate(reader):
                ...
                if progress is not None:
                    progress(ProgressEvent("render", i, meta.n_frames))
        finally:
            writer.close()
        ...
        if mux_audio(input_path, tmp_video.name, output_path):
            ...
            Path(tmp_video.name).replace(output_path)

        log.info("wrote %s (%s frames)", output_path, count)
    ...
    def _process_frame(
        ...
        target = (int(round(rendered.shape[1] * scale)), out_h)
